  ...
  const Alert = ({ name, type }) => {
    return (
      ...
      >
        ...
        <Text fontSize={"14px"} fontWeight={"700"} color={"#FF616D"}>
          {type} {name}
        </Text>
      </HStack>
    );
  ...
  function checkBloodPressure(item) {
    ...
    return (
      <HStack
        ...
        p={4}
        borderRadius={"16px"}
        bg={isHigh || isLow ? "#FFEDF0" : "#EBF8FF"}
      >
        <Icon
          boxSize={10}
          color={isHigh || isLow ? "#FF616D" : "#4A90E2"}
          as={BsHeartPulse}
        />
        <VStack align={"flex-start"}>
          <Text
            color={isHigh || isLow ? "#FF616D" : "#333"}
            fontWeight={"600"}
            fontSize={"16px"}
          >
            Blood Pressure
    ...
    <Box borderRadius={"20px"} p={6} m={2} boxShadow={"xl"} bg={"#FFFFFF"}>
      <VStack spacing={6} align={"flex-start"} width={"100%"}>
        ...
        <Text fontSize={"14px"} color={"#999"}>{`${
          data?.created &&
          moment(new Date(data.created)).format("DD/MM/YYYY hh:mm A")
        }`}</Text>
      </VStack>
    </Box>
  );
