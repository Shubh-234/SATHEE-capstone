...
import { addDoc, and, collection, doc, getDocs, onSnapshot, orderBy, query, where } from "firebase/firestore";
...
export default function RecordPage({ id, onBackClick }) {
    ...
    useEffect(() => {
        if (UserState.value.data?.email) {
            const q = query(collection(db, "record"), where("addedBy", "==", UserState.value.data?.email));
        ...
        dataLoading ?
            ...
            >
                ...
                >
                    ...
                    <AlertDialogContent p={5} borderRadius="xl" boxShadow="2xl" bg="yellow.100">
                        <AlertDialogCloseButton />
                        <AlertDialogBody>
                            <VStack alignItems="flex-start" gap={5} width="inherit">
                                ...
                                <VStack alignItems="flex-start" spacing={0} width="inherit">
                                    <Text variant="description" fontWeight="medium" color="blue.500">Blood Pressure</Text>
                                    <HStack w="inherit" justify="space-between">
                                        <HStack>
                                            <Input
                                                width="100px"
                                                value={newRecord.bpleft}
                                                onChange={(e) => setNewRecord(prev => ({ ...prev, bpleft: e.target.value, bloodpressure: `${e.target.value}/${newRecord.bpright}` }))}
                                                bg="white"
                                                borderRadius="lg"
                                                boxShadow="md"
                                                _hover={{ borderColor: "blue.300" }}
                                            />
                                            <Text fontSize="lg" fontWeight="bold" color="purple.600">/</Text>
                                            <Input
                                                width="100px"
                                                value={newRecord.bpright}
                                                onChange={(e) => setNewRecord(prev => ({ ...prev, bpright: e.target.value, bloodpressure: `${newRecord.bpleft}/${e.target.value}` }))}
                                                bg="white"
                                                borderRadius="lg"
                                                boxShadow="md"
                                                _hover={{ borderColor: "blue.300" }}
                                        ...
                                        <Text fontWeight="bold" color="purple.600">mmHg</Text>
                                    </HStack>
                                </VStack>

                                {/* Heart Rate Input */}
                                <VStack alignItems="flex-start" spacing={0} width="inherit">
                                    <Text fontWeight="medium" color="blue.500">Heart Rate</Text>
                                    <HStack w="inherit" justify="space-between">
                                        <Input
                                            width="300px"
                                            ...
                                            _hover={{ borderColor: "blue.300" }}
                                        />
                                        <Text fontWeight="bold" color="purple.600">bpm</Text>
                                    </HStack>
                                </VStack>

                                {/* Temperature Input */}
                                <VStack alignItems="flex-start" spacing={0} width="inherit">
                                    <Text fontWeight="medium" color="blue.500">Temperature</Text>
                                    <HStack w="inherit" justify="space-between">
                                        <Input
                                            width="300px"
                                            ...
                                            _hover={{ borderColor: "blue.300" }}
                                        />
                                        <Text fontWeight="bold" color="purple.600">°F</Text>
                                    </HStack>
                                </VStack>

                                {/* SpO₂ Input */}
                                <VStack alignItems="flex-start" spacing={0} width="inherit">
                                    <Text fontWeight="medium" color="blue.500">SpO₂</Text>
                                    <HStack w="inherit" justify="space-between">
                                        <Input
                                            width="300px"
                                            value={newRecord.respiratory}
                                            onChange={(e) => setNewRecord(prev => ({ ...prev, respiratory: e.target.value }))}
                                            bg="white"
                                            borderRadius="lg"
                                            boxShadow="md"
                                            _hover={{ borderColor: "blue.300" }}
                                        />
                                        <Text fontWeight="bold" color="purple.600">%</Text>
                                    </HStack>
                                </VStack>
                            </VStack>
                        </AlertDialogBody>
                        <AlertDialogFooter>
                            <Button onClick={onClose} colorScheme="red" borderRadius="full" borderColor="#DDDDDD">
                                Cancel
                            </Button>
                            <Button
                                ml={3}
